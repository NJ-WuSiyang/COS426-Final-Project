...
def shader_arms(nw: NodeWrangler, distortion):
    # Code generated using version 2.6.5 of the node_transpiler
    ...
    noise_texture = nw.new_node(Nodes.NoiseTexture,
        input_kwargs={'Vector': musgrave_texture, 'Scale': 2.0000, 'Detail': 18.0000, 'Roughness': 0.8125, 'Distortion': distortion})
...
def shader_material_002(nw: NodeWrangler, strength):
    # Code generated using version 2.6.5 of the node_transpiler
    ...
    bump_1 = nw.new_node('ShaderNodeBump',
        input_kwargs={'Strength': strength, 'Height': noise_texture.outputs["Fac"], 'Normal': bump})
...
def geometry_nodes(nw: NodeWrangler, params):
    ...
    subdivision_surface = nw.new_node(Nodes.SubdivisionSurface, input_kwargs={'Mesh': uv_sphere.outputs["Mesh"], 'Level': 2})
    
    reroute_1 = nw.new_node(Nodes.Reroute, input_kwargs={'Input': subdivision_surface})
    ...
    transform_geometry = nw.new_node(Nodes.Transform,
        input_kwargs={'Geometry': reroute_1, 'Translation': middle_translation, 'Scale': middle_scale})
    ...
    transform_geometry_9 = nw.new_node(Nodes.Transform,
        input_kwargs={'Geometry': reroute_1, 'Translation': bottom_trans, 'Scale': bottom_scale})
    
    transform_geometry_1 = nw.new_node(Nodes.Transform,
        input_kwargs={'Geometry': reroute_1, 'Translation': top_translation, 'Scale': combine_xyz_6})
    
    join_geometry = nw.new_node(Nodes.JoinGeometry,
        input_kwargs={'Geometry': [transform_geometry, transform_geometry_9, transform_geometry_1]})
    
    set_material = nw.new_node(Nodes.SetMaterial,
        input_kwargs={'Geometry': join_geometry, 'Material': surface.shaderfunc_to_material(shader_material_002, params['strength'])})
    ...
    set_material_1 = nw.new_node(Nodes.SetMaterial,
        input_kwargs={'Geometry': join_geometry_6, 'Material': surface.shaderfunc_to_material(shader_arms, params['distortion'])})
    ...
    transform_geometry_7 = nw.new_node(Nodes.Transform,
        input_kwargs={'Geometry': cylinder.outputs["Mesh"], 'Translation': hat_top_trans, 'Rotation': (0.0000, 0.0541, 0.0873)})
    ...
    transform_geometry_8 = nw.new_node(Nodes.Transform,
        input_kwargs={'Geometry': cylinder.outputs["Mesh"], 'Translation': hat_bottom_trans, 'Rotation': (0.0000, 0.0541, 0.0873), 'Scale': (1.5000, 1.5000, 0.1000)})
...
def apply(obj, params, selection=None, **kwargs):
    ...
    surface.add_material(obj, shader_material_002, selection=selection, input_kwargs={'strength': params['strength']})
...
class Snowman(AssetFactory):
    ...
    def __init__(self,
        ...
        with FixedSeed(factory_seed):
            self.params = {
                'BottomRadius': BottomRadius if (BottomRadius is not None) else np.random.uniform(0.9, 1.0),
                'BottomXYScale': BottomXYScale if (BottomXYScale is not None) else np.random.uniform(1.0, 1.2),
                'MiddleScale': MiddleScale if (MiddleScale is not None) else np.random.uniform(0.65, 0.75),
                'MiddleXYScale': MiddleXYScale if (MiddleXYScale is not None) else np.random.uniform(1.0, 1.1),
                'TopScale': TopScale if (TopScale is not None) else np.random.uniform(0.45, 0.55),
                'distortion': distortion if (distortion is not None) else np.random.uniform(0.0, 1.0),
                'strength': strength if (strength is not None) else np.random.uniform(0.1, 1.0),
            }
